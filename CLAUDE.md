...
Sesión 2026-07-21 — "Cuadros temporales" (Episode) + pase chico en Botiquín. Vía Claude Code, plan mode explícito (2 agentes Explore en paralelo para verificar el patrón real de lifeEvents/structuralZoneHistory CRUD y el flujo de guardado de síntomas/PDF/reporte, 1 agente Plan para el diseño detallado, revisión manual de archivos críticos antes de escribir el plan final → ExitPlanMode con plan aprobado). **Primera sesión con el SDK de Flutter realmente disponible en el Codespace** (`flutter 3.44.6`, confirmado con `flutter --version`) — a diferencia de casi todas las sesiones anteriores logueadas en este archivo, esta vez se corrió `flutter analyze` (limpio, cero errores nuevos en los archivos tocados; los errores preexistentes de `accordion_ui.dart`, el huérfano `app_localizations_zh_TW.dart` y `test/widget_test.dart` no son de esta sesión) y `flutter build web` completo (✓ Built build/web) antes de cerrar. No se corrió `flutter run -d chrome` interactivo — falta que Paulina pruebe el flujo real en navegador antes de confiar en la UX, mismo caveat de siempre para lo que este entorno no puede verificar.

Motivación (pedido directo de Paulina, confirmado con AskUserQuestion antes de codear): un diagnóstico agudo pero no crónico — resfrío, amigdalitis, gastritis — genera varios síntomas relacionados entre sí que hoy no tienen forma de agruparse; no es una condición permanente (`Profile.conditions`) ni un marcador puntual sin síntomas vinculados (`LifeEvent`). Decisiones confirmadas por Paulina vía AskUserQuestion: (1) modelo nuevo dedicado, no extender `conditions` ni `LifeEvent`; (2) vínculo síntoma→cuadro siempre manual (sin auto-prompt); (3) gestión completa (crear/editar/resolver/eliminar) en Ajustes → Perfil, más un atajo de creación rápida desde el mismo selector en el sheet de síntomas; (4) agrupación incluida en el Reporte in-app y el PDF en este mismo pase, no diferida. Naming propuesto por Claude, marcado explícitamente para que Paulina lo cuestione: "Cuadro" / "Cuadro temporal" en copy, clase Dart `Episode` internamente.

Implementado: `Episode` nuevo en `models.dart` (id/title/startDate/resolvedAt/note, `isResolved` getter, `copyWith` con `clearResolvedAt` — mismo patrón exacto que `StructuralEvent`), `Profile.episodes` (aditivo, mismos 3 puntos de contacto que `lifeEvents`), `SymptomEvent.linkedEpisodeId` (String? plano, mismos 4 puntos de contacto que el resto de detail layers, con `clearLinkedEpisodeId` en `copyWith`). `lib/widgets/episode_form_sheet.dart` nuevo — a diferencia de `life_event_form_sheet.dart` (su plantilla más cercana, que no usa l10n en absoluto), este sheet nuevo sí usa `context.l10n` desde el día uno, siguiendo el patrón más reciente confirmado correcto (`structural_zone_history_form_sheet.dart`) en vez de replicar la falta de localización del archivo que copia. CRUD completo en `profile_settings_screen.dart` (nueva sección "Cuadros temporales", calcada de la sección `lifeEvents`, con tag "en curso desde el {fecha}" reusando `structuralOngoingSinceTag`) + handlers `_addEpisode`/`_editEpisode` en `main_screen.dart` (mirror exacto de `_addLifeEvent`/`_editLifeEvent`). Vínculo manual en `sintomas_tab.dart`: nuevo `_episodeLinkDropdown` compartido por el sheet de agregar y el de editar síntoma — dropdown con cuadros abiertos + opción "Ninguno" + sentinela "+ Crear cuadro nuevo…" que abre el form inline y auto-selecciona el resultado; incluye manejo defensivo para que el dropdown nunca reciba un `value` sin item correspondiente (cuadro vinculado que después se resolvió, o que se eliminó) — `DropdownButton` de Flutter hace `assert` y crashea si eso pasa, no es un caso hipotético a ignorar.

Reporte y PDF: `EpisodeSection`/`EpisodeSummary`/`EpisodeSymptomOccurrence` nuevos en `clinical_report_data.dart` (mismo espíritu que `MCASSection.redFlags` — lista acotada por cuadro, no un dump crudo del período); `PdfSection.episodes` nuevo en `pdf_export_config.dart` (enum + `routineConsult()` + `PdfExportPreferences.defaultSections`); `_aggregateEpisodes()` nuevo en `pdf_report_aggregator.dart` (omite cuadros sin síntomas vinculados en el rango); render nuevo en `pdf_report_renderer.dart` (con `_pdfSafe()` en todo texto dinámico — título/nota del cuadro son texto libre del paciente, mismo tipo de bug de caracteres WinAnsi corregido en sesiones de julio); chip nuevo en `pdf_export_sheet.dart`. En `report_view.dart`: rango largo gana `_EpisodesSection` (nueva `CollapsibleSection`, independiente de `ReportTrends` porque ese servicio no tiene vínculo por-evento a episodios); rango corto (`_PeriodLog`, ≤7 días) en vez de una sección nueva agrega un tag inline "síntoma · nombre del cuadro" a las filas ya existentes, seleccionado sobre agregar una sección nueva a un modo que ya favorece el detalle día-por-día compacto.

Localización: 17 claves ARB nuevas (`episodeFormTitle/EditTitle/Subtitle/TitleHint/StartDateLabel/NoteHint/ResolvedLabel/ResolvedDateLabel`, `episodeActionSaveChanges/Create`, `symptomsEpisodeLinkLabel/None/CreateNew`, `settingsEpisodesLabel/Helper/Empty`, `settingsAddEpisodeButton`) en `app_es.arb`/`app_en.arb`/`app_zh.arb`, getters en `app_localizations.dart` + `_es`/`_en`, y en `app_localizations_zh.dart` una sola vez dentro de `AppLocalizationsZh` (no duplicadas en `AppLocalizationsZhTw`), mismo patrón confirmado correcto desde la sesión de presíncope — verificado con `python3` que las 17 claves existen idénticas en los tres locales. Deliberadamente NO localizados (para no romper la consistencia interna de esos archivos, que ya son 100% hardcoded-español hoy): los títulos de sección nuevos en `report_view.dart` y el label del chip nuevo en `pdf_export_sheet.dart` — ambos archivos ya listados como sin integración de l10n en la sesión del 18-jul; agregar un solo string localizado ahí habría sido más inconsistente que seguir la convención real del archivo. `l10n_untranslated.txt` cambió como efecto secundario esperado de correr el toolchain real (las 17 claves nuevas aparecen bajo `"zh_TW"` porque `app_zh_TW.arb` — el ARB huérfano separado — nunca las tuvo; comportamiento idéntico al de decenas de claves preexistentes, no es una regresión).

Pase chico en Botiquín (independiente, mismo pedido): `lib/widgets/med_form.dart` — 'crema' y 'sobre' agregados a la lista `_forms` (cubre tanto cremas como sobres de medicina china; un "sobre" genérico sirve para cualquier preparado en polvo/sachet sin necesitar un campo nuevo específico de "medicina china"). `lib/screens/botiquin_tab.dart` — botón compacto de ícono `+` agregado en el header de `_MedListSection` (junto al título/toggle A-Z, visible siempre, no solo cuando hay >1 medicamento), llamando al mismo `showMedFormSheet` que el botón existente de abajo — el botón grande "+ Crear medicamento" al final de la lista NO se quitó, esto es un segundo punto de entrada más rápido, no un reemplazo.

Sesión 2026-07-22 — Búsqueda + A-Z en el Baúl de síntomas, y continuación same-day para SymptomEvent. Vía Claude Code, plan mode explícito: 3 agentes Explore en paralelo (patrón A-Z/búsqueda de Botiquín; patrón completo de check-in/continuación de dolor estructural; ciclo de vida de SymptomEvent y su impacto en frecuencia/reporte/PDF/flare detection) para verificar contra el código real antes de escribir el plan, más una ronda de AskUserQuestion tanto para acotar la tarea 1 (qué menú, alfabetizar/filtrar/ambos) como para resolver el riesgo de diseño real de la tarea 2 (ver abajo) → ExitPlanMode con plan aprobado.

(1) Baúl de síntomas — búsqueda + A-Z, mismo patrón que ya existía en Botiquín (campo de búsqueda transitorio + toggle A-Z persistente en `optionalTrackers`). Segunda vez que se necesita este patrón exacto — en vez de duplicar los widgets privados `_SearchField`/`_SortToggleButton` de `botiquin_tab.dart` una segunda vez, se extrajeron a `lib/widgets/search_field.dart`/`lib/widgets/sort_toggle_button.dart` (clases públicas `SearchField`/`SortToggleButton`), mismo criterio ya usado para `StructuralChip` (duplicar hasta que aparece un segundo host real, entonces extraer). `botiquin_tab.dart` pasó a importar y usar las versiones compartidas, sin cambio de comportamiento — al hacerlo quedó un import de `app_localizations.dart` sin uso (ya no había ninguna referencia directa al tipo `AppLocalizations` en ese archivo tras la extracción), corregido en la misma sesión. Toggle nuevo `optionalTrackers['symptom_vault_sort_alpha']`; igual que Botiquín, el orden A-Z solo ordena la lista transitoria de render, nunca `Profile.symptomVault` (que sigue siendo "más reciente primero", el orden real que usa `_addSymptomToVault`'s `insert(0, ...)`).

(2) Continuación same-day para SymptomEvent — pedido explícito de Paulina: un síntoma que mejora pero no llega a resolverse del todo, al volver a registrarse, debe tratarse como continuación del registro inicial de ese día en vez de una entrada nueva. Ya existía exactamente este concepto para dolor estructural (`StructuralEvent.resolvedAt`/`comparedToUsual` + `structural_checkin_sheet.dart`, Sesión 2026-07-18), pero portarlo tal cual a síntomas generales del baúl tenía un riesgo real identificado durante la exploración: a diferencia de una zona con una lesión física específica, la mayoría de los síntomas del baúl (fatiga, náuseas, niebla mental) son recurrentes día a día por naturaleza — el patrón estructural (cualquier evento sin resolver, de cualquier día anterior, dispara check-in) habría fusionado silenciosamente registros de días distintos que en realidad son episodios separados, y además habría hecho que todo `SymptomEvent` histórico apareciera "sin resolver" por default (todo dato viejo tiene `resolvedAt == null`). Presentado a Paulina como pregunta directa (AskUserQuestion, tres opciones: solo mismo día / multi-día automático como estructural / multi-día preguntando primero) — eligió **solo mismo día**, lo que de paso vuelve el campo nuevo seguro de agregar sin ninguna migración: un evento viejo con `resolvedAt == null` nunca importa porque el gate solo mira el día actualmente seleccionado (`widget.selectedDate`, no `DateTime.now()` — confirmado contra `_timestampForLog()` antes de codear, dado que la app permite registrar retroactivamente).

Verificación de terminología que evitó un bug real antes de codear: `SymptomSeverity.none` (valor 0, "Ninguna") ya está sobrecargado en el código como el sentinela de "registrar sin calificar" (`_isUnrated`/`unratedSentinel` en `_openSeverityMenu`), no como "resuelto/ausente" — de haber usado severidad 0 como señal de resolución (la lectura ingenua de "eventos que mejoran pero no han llegado a 0" del pedido original) habría colisionado con ese significado ya existente. "Resuelto" quedó modelado como acción explícita separada (`resolvedAt`), igual que estructural.

Implementado: `SymptomEvent.resolvedAt`/`isResolved` nuevos en `lib/models/models.dart` (mismo patrón `copyWith(resolvedAt, clearResolvedAt)` que `StructuralEvent`). Divergencia deliberada respecto al patrón estructural: en vez de un enum de tendencia tipo `comparedToUsual` (necesario ahí porque `severity` es opcional en el camino quick-log de `StructuralEvent`), el check-in de `SymptomEvent` reutiliza directamente `SeverityDotPicker` — un síntoma general siempre tiene `severity`, así que la trayectoria de mejora/empeoramiento queda expresada por el propio valor elegido, sin necesitar un campo nuevo de tendencia. `lib/widgets/symptom_checkin_sheet.dart` nuevo (`showSymptomCheckInSheet`/`SymptomCheckInResult`) — usa `SeverityDotPicker(anchor: currentSeverity, ...)` para mostrar el valor anterior como referencia translúcida mientras se elige el nuevo, mismo parámetro `anchor` que ya usaba el outcome card de seguimiento en `hoy_tab.dart` (`ActionEffectivenessDialog`), no una idea nueva. Gate nuevo al inicio de `_openSeverityMenu` (`sintomas_tab.dart`): si ya existe un `SymptomEvent` con el mismo `name` sin resolver en `getSymptomsForDay(widget.selectedDate)`, abre `_openSymptomCheckIn` (nuevo) en vez del flujo de registro nuevo — cubre uniformemente todos los síntomas que pasan por este método, incluidos los siete con detail layer (cefalea/fatiga/abdominal/presíncope/pélvico/torácico/MCAS), ya que todos bifurcan ahí antes de `saveWith`. Decisión de alcance explícita: el check-in solo actualiza `severity`/`resolvedAt`, nunca reabre el sheet de detalle estructurado — para eso sigue existiendo la edición completa vía long-press (`_editSymptomEvent`), sin cambios. El `timestamp` original nunca se toca en el check-in, mismo criterio que estructural (preserva la hora real del primer registro). "Registros de hoy" gana un ícono de check-in (`Icons.check_circle_outline`, mismo que estructural) antes del botón ✕, visible solo si `!event.isResolved`, y un sufijo `' ✓'` en el label cuando ya está resuelto — a diferencia de estructural, aquí no hay concepto de "carried over" ni se oculta nunca el botón de borrar, porque el alcance same-day no lo necesita. `_editSymptomEvent` gana un `CheckboxListTile` "Marcar como resuelto" para corregir/resolver retroactivamente sin pasar por el check-in.

Efecto secundario esperado y documentado, no bloqueante: un síntoma revisado dos veces el mismo día pasa de contar 2 registros a contar 1 en `symptomFrequencyStats` (dashboard T0.3), `report_trends.dart`, `pdf_report_aggregator.dart` (`occurrences`/`severeSymptomDays`) y en la ventana de 24h de `flare_detection_service.dart` (Rule 1, acumulación de síntomas severos) — mismo comportamiento ya aceptado para `StructuralEvent` (que tampoco se cuenta dos veces por check-in), y si acaso más correcto: evita inflar el conteo solo por re-chequear el mismo síntoma varias veces el mismo día. No requirió ningún cambio en esos servicios.

8 claves ARB nuevas (`symptomsVaultSearchHint`/`NoResults`, `symptomsEditResolvedLabel`, `symptomCheckInTitle`/`Subtitle`/`ResolvedButton`/`Skip`) en `app_es.arb`/`app_en.arb`/`app_zh.arb`. **Primera sesión que corrió el flujo de localización real de punta a punta**: en vez del patrón habitual de estas últimas semanas (editar a mano los `.dart` generados, sin `flutter gen-l10n` disponible), esta vez se corrió `flutter gen-l10n` real sobre los ARB editados — confirmó de paso que el patrón ya establecido a mano en sesiones anteriores ("agregar claves nuevas una sola vez en `AppLocalizationsZh`, heredadas por `AppLocalizationsZhTw`, sin duplicar en ambas clases") es exactamente lo que el generador real produce, no una convención inventada. `flutter analyze` limpio salvo los lints preexistentes ya conocidos (`accordion_ui.dart`, el huérfano `app_localizations_zh_TW.dart`, `test/widget_test.dart`) más el import no usado en `botiquin_tab.dart` mencionado arriba (causado por esta sesión, corregido en la misma sesión). **Compilación confirmada por Paulina** ("todo compila bien"); procederá a desplegar este ciclo — pendiente su confirmación del deploy real para registrar la línea correspondiente en CHANGELOG.md § "Estado de Deploy". `pubspec.yaml` sigue en `2.3.5+1` (sin bump todavía — por convención de este archivo, el bump ocurre recién cuando se confirma el deploy real, no antes).
