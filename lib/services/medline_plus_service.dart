...
class MedlinePlusService {
  ...
  Future<_ConditionMapping?> resolveCondition(String userInput) async {
    ...
    // Loose contains-match fallback for things like "EDS tipo hipermóvil".
    for (final m in _mappings!) {
      for (final alias in m.aliases) {
  ...
  /// Returns null on network failure with no cached fallback.
  Future<MedlinePlusContent?> getContent(String icd10) async {
    ...
    final url = Uri.parse('$_base?'
        ...
        '&mainSearchCriteria.v.c=${Uri.encodeComponent(icd10)}'
    ...
    try {
      ...
      // First entry is the canonical match.
      ...
      final content = MedlinePlusContent(
        icd10: icd10,
        title: title.trim(),
  ...
  /// Quick HTML strip for the summary field — MedlinePlus returns light
  /// HTML even in JSON wrapping.
