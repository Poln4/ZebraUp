#!/usr/bin/env python3
"""
ZebraUp — Phase 5.2d.3b: lib/services/fever_analysis.dart (new file)
====================================================================

Creates the FeverAnalysis service:

  - FeverSiteLocalization extension (moved here from fever_form_sheet.dart
    so non-form-sheet callers can use it without weird imports)
  - FeverTrend enum (rising/falling/steady)
  - LatestFeverInfo class (latest reading + previous + trend getter)
  - FeverEpisode class (contiguous cluster of feverish readings)
  - FeverAnalysis static class with:
      * feverThresholdC = 37.5°C (conservative, covers axillary)
      * highFeverThresholdC = 38.0°C
      * episodeGapHours = 48
      * activeEpisodeHours = 48
      * chipMaxAgeHours = 24
      * latestForChip(readings) -> LatestFeverInfo?
      * detectEpisodes(readings) -> List<FeverEpisode>

Run from the repo root. Refuses to overwrite an existing file.
"""

import sys
from pathlib import Path

TARGET = Path("lib/services/fever_analysis.dart")
...
def main():
    ...
    print(f"  - FeverSiteLocalization extension (moved here)")
    print(f"  - FeverTrend enum, LatestFeverInfo, FeverEpisode classes")
    print(f"  - FeverAnalysis with latestForChip + detectEpisodes")
